  ...
  run: async (payload: { prompt: string; roomId: string; userId: string }) => {
    ...
    try {
      ...
      try {
        ...
        if (flow?.nodes) {
          Object.keys(flow.nodes as object).forEach((id) => existingNodeIds.add(id));
        }
        if (flow?.edges) {
          existingEdgeCount = Object.keys(flow.edges as object).length;
        }
        if (nodeCount > 0) {
      ...
      const finalNodeIds = new Set<string>(existingNodeIds);
      const validActionCalls: ToolCall[] = [];
      ...
      for (const call of actionCalls) {
        ...
        if (call.toolName === "addNode") {
          ...
          const { id, source, target } = input as { id: string; source: string; target: string };
          if (!id || !source || !target) {
      ...
      console.error("[Graph] Error during execution, falling back to minimal valid graph:", error.message || error);
      try {
