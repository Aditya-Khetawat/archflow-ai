import {
  ...
  tool,
  withRetry
} from "../../../../chunk-T4CYUGEW.mjs";
import "../../../../chunk-2RL2RZBH.mjs";
import {
  external_exports,
  task
} from "../../../../chunk-M5SZ2SHA.mjs";
...
async function fetchPolyfill() {
  return typeof globalThis.fetch !== "undefined" ? globalThis.fetch : (await import("../../../../lib-5FRZQI4G.mjs")).default;
}
...
// trigger/design-agent.ts
var AI_USER_ID = "archflow-ai";
var AI_USER_INFO = { name: "ArchFlow AI", avatar: "", color: "#6457f9" };
...
var designAgent = task({
  id: "design-agent",
  retry: { maxAttempts: 1 },
  // Trigger.dev retry disabled — withRetry() handles retries with correct delay
  run: /* @__PURE__ */ __name(async (payload) => {
    ...
    try {
      ...
      try {
        ...
        if (flow?.nodes) {
          Object.keys(flow.nodes).forEach((id) => existingNodeIds.add(id));
        }
        if (flow?.edges) {
          existingEdgeCount = Object.keys(flow.edges).length;
        }
        if (nodeCount > 0) {
...
Extend or modify based on the request; only clear if explicitly asked.`;
      ...
      const result = await withRetry(async () => {
        const tsRequest = (/* @__PURE__ */ new Date()).toISOString();
...
${canvasContext}`,
          ...
          const tsResponse = (/* @__PURE__ */ new Date()).toISOString();
          console.log(`[GEMINI_TIMELINE] design-agent RESPONSE received | ts: ${tsResponse}`);
          return r;
        } catch (innerErr) {
          const tsFailure = (/* @__PURE__ */ new Date()).toISOString();
      ...
      console.log("[Parser] Tool calls received", { count: toolCalls.length });
      if (toolCalls.length === 0) {
        console.warn("[Parser] Warning: No tool calls generated by Gemini.");
        throw new Error("AI did not generate any design actions. Please refine your prompt.");
      }
      for (const call of toolCalls) {
        if (!call.toolName || !call.input) {
          console.error("[Parser] Invalid tool call structure:", call);
          throw new Error("AI generated a malformed tool call.");
        }
      }
      console.log("[Parser] JSON valid");
      ...
      const validActionCalls = [];
      let finalEdgesCount = existingEdgeCount;
      for (const call of actionCalls) {
        const input = call.input;
        if (call.toolName === "addNode") {
          const { id, label } = input;
          if (!id || !label) {
            console.warn("[Graph] Skipping invalid addNode tool call: missing id or label", call);
            continue;
          }
          finalNodeIds.add(id);
          validActionCalls.push(call);
        } else if (call.toolName === "deleteNode") {
          const { id } = input;
          finalNodeIds.delete(id);
          validActionCalls.push(call);
        } else if (call.toolName === "addEdge") {
          const { id, source, target } = input;
          if (!id || !source || !target) {
            console.warn("[Graph] Skipping invalid addEdge tool call: missing id, source, or target", call);
            continue;
          }
          if (!finalNodeIds.has(source)) {
            console.warn(`[Graph] Skipping addEdge "${id}": source node "${source}" does not exist in graph.`, call);
            continue;
          }
          if (!finalNodeIds.has(target)) {
            console.warn(`[Graph] Skipping addEdge "${id}": target node "${target}" does not exist in graph.`, call);
            continue;
          }
          validActionCalls.push(call);
          finalEdgesCount++;
          ...
          if (!finalNodeIds.has(id)) {
            console.warn(`[Graph] Skipping ${call.toolName}: node "${id}" does not exist.`, call);
            continue;
          }
          validActionCalls.push(call);
        }
      }
      const addCount = validActionCalls.filter((c) => c.toolName === "addNode").length;
      console.log(`[Graph] Valid actions to apply: ${validActionCalls.length} (adds: ${addCount})`);
      console.log(`[Graph] Final graph to render: ${finalNodeIds.size} nodes, ${finalEdgesCount} edges`);
      await lb.broadcastEvent(payload.roomId, {
        type: "ai-status",
      ...
      await lb.mutateStorage(payload.roomId, ({ root }) => {
        ...
        for (const call of validActionCalls) {
          applyToolCall(call, nodes, edges);
        }
      });
      console.log("[Graph] Committing final graph to Liveblocks complete");
      await lb.broadcastEvent(payload.roomId, {
        type: "ai-status",
        message: summary,
        status: "complete"
      ...
      return { success: true, actionsApplied: validActionCalls.length, summary };
    } catch (error3) {
      console.error("[Graph] Error during execution, falling back to minimal valid graph:", error3.message || error3);
      try {
        const fallbackSummary = "Rendered default architecture diagram due to generation failure.";
        console.log("[Graph] Writing fallback minimal graph directly to Liveblocks storage...");
        await lb.mutateStorage(payload.roomId, ({ root }) => {
          const flow = root.get("flow");
          if (!flow) return;
          const nodes = flow.get("nodes");
          ...
          const gatewayColor = NODE_COLORS[0];
          const gatewaySize = SHAPE_DEFAULTS.rectangle;
          nodes.set(
            "api-gateway",
            LiveObject.from(
              {
                ...
                data: { label: "API Gateway", color: gatewayColor.fill, textColor: gatewayColor.text, shape: "rectangle" },
                width: gatewaySize.width,
                height: gatewaySize.height
              },
              NODE_SYNC_CONFIG
            )
          );
          const dbColor = NODE_COLORS[7];
          const dbSize = SHAPE_DEFAULTS.cylinder;
          nodes.set(
            "database",
            LiveObject.from(
              {
                ...
                data: { label: "Database", color: dbColor.fill, textColor: dbColor.text, shape: "cylinder" },
                width: dbSize.width,
                height: dbSize.height
              },
              NODE_SYNC_CONFIG
            )
          );
        ...
        console.log("[Graph] Fallback minimal graph applied successfully.");
        return { success: true, actionsApplied: 3, summary: fallbackSummary };
      } catch (fallbackError) {
        console.error("[Graph] Failed to apply fallback graph:", fallbackError.message || fallbackError);
      }
      await lb.broadcastEvent(payload.roomId, {
        type: "ai-status",
